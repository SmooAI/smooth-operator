...
Config parsing is tolerant — a malformed workflow or config degrades to the server default and never crashes a session. The judge is failure-tolerant — any judge error leaves the conversation on the current step. Delivery seam: `ServerState.agent_configs` maps `agentId → AgentConfig`, resolved per turn from the session's agent (empty → behavior unchanged). Mirrors the Rust reference PR.
