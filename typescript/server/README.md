...
but flips the host to `0.0.0.0`, since the process default of `127.0.0.1` is
...
> This host's pre-parity `SMOOTH_OPERATOR_HOST` / `SMOOTH_OPERATOR_PORT` (and
> `SMOOAI_MODEL` for the model id) still work as aliases — the canonical name wins
> when both are set.
