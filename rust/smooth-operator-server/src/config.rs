...
/// was 0.0 — but a growing set of frontier models reject any temperature
/// but their default and 400 the entire request:
///
/// ```text
...
/// ```
///
/// The symptom is nasty because it doesn't look like a config error: the
/// server boots, accepts the turn, every LLM call 400s, and the user
/// sees an assistant that simply says nothing. A model picker offering
/// these models is a no-op or worse.
///
...
/// temperature-0 determinism on models that would allow it, which is a
/// fair trade against "only the default model works at all".
///
/// The cleaner long-term shape is `Option<f32>` on `LlmConfig` so we send
/// nothing and take each provider's default.
...
mod tests {
    ...
    /// in a test so a future "make it deterministic again" change has to
    /// read why it isn't: gpt-5.1/5.5, claude-opus-4-7/4-8,
    /// claude-sonnet-5 and others 400 the whole request on anything else,
