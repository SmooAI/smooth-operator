...
- ✅ **Auth + admin API (increment 1)** — `Role` (admin/curator/basic, ordered) + `Principal` + the `AuthVerifier` seam with four modes: BYO **SST OpenAuth** JWT (`AUTH_MODE=jwt`, HS256 + RS256), **Smoo identity** (`AUTH_MODE=smoo`, hosted; live-introspection stubbed), **tokenless trusted-upstream** (`AUTH_MODE=trusted`, proxied-integration — the upstream forwards a `base64url(JSON)` identity in the token slot, trusted WITHOUT signature/`exp`; fails closed to anonymous on bad input, loud startup warning; see [ACCESS-CONTROL.md](ACCESS-CONTROL.md#tokenless-auth_modetrusted-proxied-integration)), and dev-only `NoAuthVerifier` (`AUTH_MODE=none`). Secure-by-default (`jwt` default; keyless `jwt`/`smoo` refuses to start — never silent no-auth; `trusted`/`none` only by explicit opt-in). Auth-gated admin HTTP API on the axum service (`/admin/me`, `/admin/conversations`(+`/{id}/messages`), `/admin/indexing/runs`, `/admin/document-sets`, public `/admin/health`) with org-scoping + "Basic sees own". See [ADMIN-API.md](ADMIN-API.md). RBAC layers on org scoping + `DocAcl`.
