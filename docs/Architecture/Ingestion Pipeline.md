...
analog of [[Feature Gaps|a mature connector framework]] and closes
...
| `source`   | origin label or URL — for the GitHub connector the blob/issue **URL**, which the runtime surfaces as a citation `url` on `eventual_response` (see [[Connectors]] and [[Protocol Reference#citations-on-eventual_response|PROTOCOL.md]]); for others a label (`"file"`, `"web"`, …) |
...
| `acl`      | optional access-control labels — stamped into the doc's `acl_v2` `DocAcl` (as group entitlements) and enforced at read; see [[Access Control]] |
...
- **`GithubConnector::new(config)`** — pulls a GitHub repository's **prose**
  ...
  [[Connectors]]** (content types, `GithubAuth` App-vs-PAT, the
...
ACL-filtered retrieval enforces at read — see [[Access Control]].
...
- [[Storage Adapters]] — the `StorageAdapter` seam and its knowledge slice
- [[Tools]] — `fetch_url` (whose SSRF guard + HTML stripper the web connector reuses) and `knowledge_search` (retrieval over what ingestion stores)
- [[Feature Gaps]] — G1/G2/G9 this closes
