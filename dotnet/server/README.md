...
- **Fast-model preamble** (`SMOOTH_AGENT_PREAMBLE_MODEL`, off by default): a small fast model runs IN PARALLEL with the turn and emits ONE ephemeral `stream_preamble` sentence ("what I'm about to do") to cover the main model's time-to-first-token. Same gateway/key as the turn, model id + a 64-token cap overridden. It is never awaited, dropped the moment the real answer starts streaming, never persisted, and never part of `eventual_response`; any failure is logged at debug and swallowed.
...
- **Turn cancellation (the "Stop button")**: a `cancel` frame aborts the connection's in-flight turn — the turn's `CancellationTokenSource` is cancelled, the partial assistant message is discarded (never persisted; the user's message stays), and a terminal `cancelled` event (status `499`, echoing the turn's `requestId`) replaces the `eventual_response`. One active turn per connection: a second `send_message` mid-turn is rejected with `TURN_IN_PROGRESS`. A client disconnect aborts the turn too; a graceful shutdown still drains it.
- **`/admin/*` API + deployable host**: `MapSmoothOperatorAdmin()` mounts the full management API the [console](https://github.com/SmooAI/smooth-operator/tree/main/console) drives — the same routes, camelCase shapes, `{"error":{"code","message"}}` envelope and role gate (basic → curator → admin) that the Rust, Go, TypeScript and Python servers ship: `/admin/health` (ungated), `/admin/me`, `/admin/conversations[/{id}/messages]`, `/admin/indexing/runs`, `/admin/document-sets`, `/admin/connectors` CRUD + `/{id}/index`, and `/admin/settings` (GET/PUT). Auth fails closed (401 without a bearer token, 403 below the required role); `AUTH_MODE=none` resolves to an Admin principal so the console renders against a local server. Connector configs, settings and indexing runs live in an in-memory `AdminStores` (register your own instance in DI to pre-seed or share that state; durable storage means changing that class, not registering a different one). `GET /admin/model-costs` is ungated too (pricing is not org-sensitive) and maps the gateway's `/model/info` into per-model cost badges, fetched once per process and degrading to `{}` on any gateway error — an omitted field stays `null` rather than defaulting, since a `0` cost would render a free-model badge on a paid model. `POST /admin/publish` pushes a realtime event to a connection over the `IBackplane` registry, reporting a truthful `delivered` count of 0 or 1; session / user / org / agent targets answer a hard `501 UNSUPPORTED_TARGET` with no `delivered` field at all, because a connection-id registry cannot route them and `{"delivered": 0}` would read as "accepted, reached nobody". `POST /admin/reindex` is this host's own extra — re-ingest the configured GitHub repos without a restart. `SmooAI.SmoothOperator.Server.Host` is a runnable ASP.NET app (model + storage + auth + startup ingestion from env config) shipped as a container via a `Dockerfile`.
